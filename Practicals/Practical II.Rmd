...
```{r}
# CF: not working when tried on 3/8/2023
# summary(with(imp1, mean(tv)))
# CF: my version that seems to work
lapply(with(imp1, tv)$analyses, mean)
```

This just returns means of imputed values. Does not include the original data. 

```{r}
colMeans(imp1$imp$tv)
```

CF: mean of `tv` within the 5 imputed data sets using complete data.

```{r}
dslist <- complete(imp1, "all", include = FALSE)
lapply(dslist, function(x)mean(x[["tv"]]))
```

CF: `summary()` of tv in the 5 imputed data sets

```{r}
lapply(dslist, function(x)summary(x[["tv"]]))
```
